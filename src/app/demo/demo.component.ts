...
export class DemoComponent implements OnInit {
  ...
  userEnrollEventDetails: any =
    userEnrollEventDetailsMock.userEnrollEventDetailsMock;
  ...
  ngOnInit() {
    ...
    this.eventDetailService.getEvent("do_213441736769036288135").subscribe(
      (data: any) => {
    ...
    console.log("PPPPPPPPPPPP - ", this.p);
  }
  ...
  showMyEventListPage() {
    ...
    this.eventListService.getMyEventList(this.userId).subscribe((data: any) => {
      ...
      if (eventsList.length != 0) {
        ...
        this.eventListService.getEventList(this.Filterdata).subscribe(
          (data) => {
            if (data.responseCode == "OK") {
  ...
  getFilteredData(event) {
    ...
    if (event.search) {
      ...
      event.filtersSelected.eventTime &&
      event.filtersSelected.eventType
    ) {
    ...
    this.eventListService
      .getEventList(this.Filterdata, this.query, this.sort_by)
      .subscribe(
        (data) => {
          if (data.responseCode == "OK") {
            this.isLoading = false;
            ...
            for (var k in tempEventList) {
              ...
              var dateTime: any;
              dateTime =
                this.todayDate + " " + dTime.toLocaleTimeString() + "+05:30";
                  ...
                  case "Upcoming":
                    ...
                    if (
                      ...
                      tempEventList[k].startDate +
                        "-" +
                        tempEventList[k].startTime >
                        this.todayDate + "-" + timeTemp
                    ) {
                      tempEventListData.push(tempEventList[k]);
                    }
                    //  tempEventListData.push(tempEventList[k]);
